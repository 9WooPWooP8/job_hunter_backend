...
async def valid_applicant_create(
    user: ApplicantCreate, user_service: Annotated[UserService, Depends(UserService)]
) -> ApplicantCreate:
...
async def valid_recruiter_create(
    user: RecruiterCreate, user_service: Annotated[UserService, Depends(UserService)]
) -> RecruiterCreate:
    existing_user_by_email = await user_service.get_user_by_email(user.email)
    print("bfalskdjflaskjdf", existing_user_by_email)
    if existing_user_by_email:
