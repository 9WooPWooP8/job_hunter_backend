...
from src.resumes.models import (
    Contact,
    Education,
    EmploymentRecord,
    Resume,
...
class ResumeService:
    ...
    async def create_resume(self, resume: ResumeRequest, user: Applicant) -> Resume:
        ...
        for education in resume.education:
            new_education = ResumeEducation(
                education_id=education.education_id, end_year=education.end_year
            )
    ...
    async def update_resume(
        self, resume_id: int, updated_resume: ResumeRequest, user: Applicant
    ) -> Resume:
        ...
        for record in updated_resume.employment_records:
            existing_employment_records = list(
                filter(lambda x: x.id == record.id, db_resume.employment_records)
            )
        ...
        for record in db_resume.employment_records:
            deleted_employment_record = list(
                filter(lambda x: x.id == record.id, updated_resume.employment_records)
            )
        ...
        for contact in updated_resume.contacts:
            existing_contacts = list(
                filter(lambda x: x.contact_id == contact.contact_id, db_resume.contacts)
            )
            if existing_contacts:
                ...
                new_contact = ResumeContact(
                    extra_data=contact.extra_data, contact_id=contact.contact_id
                )
                db_resume.contacts.append(new_contact)
        deleted_contacts = []
        for contact in db_resume.contacts:
            deleted_contact = list(
                filter(
                    lambda x: x.contact_id == contact.contact_id,
                    updated_resume.contacts,
                )
            )
        ...
        for education in updated_resume.education:
            existing_education = list(
                filter(
                    lambda x: x.education_id == education.education_id,
    ...
    async def upload_photo(
        self, resume_id: int, file: UploadFile, user: Applicant
    ) -> Resume:
