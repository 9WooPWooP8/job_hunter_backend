...
class SearchService:
    ...
    async def find_company(
        self, filter: CompaniesFilterSearchRequest, limit, page
    ) -> CompaniesFilterSearchResponse | None:
        ...
        select_query = (
            ...
            .filter(
                or_(
                    Company.name.like(company_name_search),
        ...
        return {
            "companies": await fetch_all(self.db, select_query),
            "limit": limit,
            "page": page,
            "total": total,
        }
    ...
    async def find_vacancies(
        self, filter: VacancyRequest, limit, page
    ) -> VacancySearchResponse | None:
        pagination = self.calculate_pagination(limit, page)

        vacancies_name_description = "%{}%".format(filter.description)

        select_query = select(Vacancy).filter(
            or_(
                Vacancy.description.like(vacancies_name_description),
        ...
        if filter.experience_min > 0:
            select_query = select_query.filter(
                filter.experience_min >= Vacancy.experience_min
            )

        if filter.experience_max > 0:
            select_query = select_query.filter(
                filter.experience_max <= Vacancy.experience_max
            )
        ...
        return {
            "vacancies": await fetch_all(self.db, select_query),
            "limit": limit,
            "page": page,
            "total": total,
        }
    ...
    async def find_resumes(
        self, filter: ResumeRequest, limit, page
    ) -> ResumesSearchResponse | None:
        pagination = self.calculate_pagination(limit, page)

        resumes_description = "%{}%".format(filter.description)
        job_title = "%{}%".format(filter.job_title)

        select_query = select(Resume).filter(
            or_(
                Resume.description.like(resumes_description),
        ...
        if filter.applicant_id:
            select_query = select_query.filter(
                filter.applicant_id == Vacancy.applicant_id
            )
        ...
        return {
            "resumes": await fetch_all(self.db, select_query),
            "limit": limit,
            "page": page,
            "total": total,
        }
